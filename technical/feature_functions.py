...
def heikenashi(prices, periods):
    """"
    :param prices: dateframe of OHLC & volume data
    :param periods: periods for which to create the candle
...
def detrend(prices, method='difference'):
    """
    :param prices: dataframe of CandleStick currency Data
...
def fseries(x, a0, a1, b1, w):
    ...
    :return: the vlaue of the fourier function
    """
...
def fourier(prices, period, method='difference'):
    """
    :param prices: CandleStick dataframe
    :param period: list of periods for which to compute coefficients [3,5,10,...]
    :param method: method by which to detrend the data
    :return: dic of dataframes containing coefficients for said periods
    """
    ...
    for i in range(0, len(period)):
        ...
        for j in range(period[i], len(prices) - period[i]):
            ...
            coeff = np.append(coeff, res[0], axis=0)

        warnings.filterwarnings('ignore', category=np.VisibleDeprecationWarning)

        coeff = np.array(coeff).reshape(((len(coeff) // 4), 4))

        df = pd.DataFrame(coeff, index=prices.iloc[period[i]:-period[i]])
    ...
    results.coefficient = dict

    return results

def sine(prices, period, method='difference'):
    """
    :param prices: CandleStick dataframe
    :param period: list of periods for which to compute coefficients [3,5,10,...]
    :param method: method by which to detrend the data
    ...
    for i in range(0, len(period)):
        ...
        coeff = np.array(coeff).reshape((len(coeff) // 3, 3))
...
def OHLCresample(DataFrame, TimeFrame, column='ask'):
    ...
    if np.any(DataFrame.columns == 'ask'):
        ...
        vol = grouped['vol'].resample(TimeFrame).ohlc()
...
def adosc(prices, periods):
    ...
    :param prices: OHCL price data
    :param periods: list of periods to calculate function values
    :return: indicator vlaues for indicated periods
    """
...
def macd(prices, periods):
    ...
    :param prices: OHLC dataframe
    :param periods: 1x2 array contianing vlaues for the EMAs
